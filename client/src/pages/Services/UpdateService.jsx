...
const UpdateService = () => {
  ...
  const handleUpdate = async (e) => {
    ...
    updatedService.price = parseFloat(updatedService.price);
    updatedService.addedDate = new Date().toISOString();
    updatedService.userEmail = user.email;

    fetch(`${import.meta.env.VITE_API_URL}/update-service/${id}`, {
      method: 'PATCH',
      credentials: 'include',
      headers: {
  ...
  return (
    <section className="w-11/12 md:w-2/3 mx-auto my-12 bg-white p-6 shadow-lg rounded-xl">
      {/* Title */}
      <h2 className="text-2xl md:text-3xl font-bold text-center text-gray-700 mb-4">
        Update <span className="text-primary">Service</span>
      </h2>
      <p className="text-center text-gray-500 mb-6 max-w-2xl mx-auto">
        Update the service details below to keep your listing current.
      </p>
      ...
      <form onSubmit={handleUpdate} className="space-y-4">
        ...
        <div>
          ...
          <input type="text" name="image"
            ...
            className="w-full text-xs md:text-lg px-4 py-2 text-gray-700 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary"
            required defaultValue={service?.image} />
        </div>
        ...
        <div>
          ...
          <input type="text" name="company" placeholder="e.g., Creative Studio"
            className="w-full text-xs md:text-lg px-4 py-2 text-gray-700 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary"
            required defaultValue={service?.company} />
        </div>
        ...
        <div>
          ...
          <input type="url" name="website" placeholder="https://"
            className="w-full text-xs md:text-lg px-4 py-2 text-gray-700 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary"
            required defaultValue={service?.website} />
        </div>
        ...
        <div>
          ...
          <textarea name="description" rows="5"
            ...
            className="w-full text-xs md:text-lg px-4 py-2 text-gray-700 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary"
            required defaultValue={service?.description}></textarea>
        </div>
        ...
        <div>
          ...
          <select name="category"
            ...
            required defaultValue={service?.category}>
        ...
        <div>
          ...
          <input type="number" name="price"
            ...
            min="0"
            className="w-full text-xs md:text-lg px-4 py-2 text-gray-700 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-secondary"
            required defaultValue={service?.price} />
        </div>
        ...
        {/* Submit */}
        <button type="submit" className="w-full btn btn-primary hover:btn-primary-focus text-white text-sm md:text-lg">
            Update Service
        </button>
      </form>
    </section>
